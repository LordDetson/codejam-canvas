...
let rgba = "rgba(${red},${green},${blue},${saturation})";

function draw4x4() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    let data = JSON.parse(matrix4x4);
    let widthRect = canvas.width / 4;
    let heightRect = canvas.height / 4;
    let x = 0;
    let y = 0;
...
function draw32x32() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    let data = JSON.parse(matrix32x32);
    let widthRect = canvas.width / 32;
    let heightRect = canvas.height / 32;
    let x = 0;
    let y = 0;
    for (let i = 0; i < data.length; i++) {
        x = 0;
